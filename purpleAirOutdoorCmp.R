...
## download_date = "093018"
## download_date = "100718"
## download_date = "101418"
download_date = "102118"
files = list.files(path=sprintf("~/Dropbox/ROCIS/DataBySensor/PurpleAir/download/%s/", download_date),
                   pattern = ".csv")
head(files)

summaryDf = data.frame(filename=files)
summaryDf <- summaryDf %>%
  dplyr::mutate(nameOnMap = substr(filename, 1, regexpr(" \\(", filename) - 1),
                lat = substr(filename, regexpr("\\(", filename) + 1, regexpr("-", filename) - 2),
                lng = substr(filename, regexpr("-", filename), regexpr("\\)", filename) - 1),
...
summaryByType = function(fileType) {
  ...
  for (f in byTypeFiles) {
    ...
    df = readr::read_csv(sprintf("~/Dropbox/ROCIS/DataBySensor/PurpleAir/download/%s/%s", download_date, f)) %>%
      as.data.frame() %>%
